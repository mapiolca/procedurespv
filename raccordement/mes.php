...
$mesTransitions = array(
	...
	'mark_blocked' => (int) $object->mes_required === 1 && in_array((int) $object->mes_status, array(1, 2, 3, 4), true),
	'mark_canceled' => (int) $object->mes_required === 1 && in_array((int) $object->mes_status, array(1, 2, 3, 4, 5), true),
);
...
if (in_array($action, $sensitiveActions, true)) {
	...
	$uploadResult = array('result' => 0, 'filename' => '', 'error' => '');
	$oldConsuelPiece = null;
	$oldConsuelFile = '';
	$saveError = false;
	if ($action === 'save') {
		$oldConsuelPiece = $workflow->getConsuelPiece($object);
		if (is_object($oldConsuelPiece) && $workflow->pieceFileExists($oldConsuelPiece)) {
			$oldConsuelFile = rtrim((string) $oldConsuelPiece->filepath, '/').'/'.(string) $oldConsuelPiece->filename;
		}
		$db->begin();
		...
		$uploadResult = procedurespvStoreRaccordementUpload(
			'consuel_file',
			$object,
			RaccordementWorkflow::CONSUEL_PIECE_CODE,
			$langs->transnoentitiesnoconv('ConsuelDocument'),
			RaccordementWorkflow::CONSUEL_PIECE_ORIGIN,
			1
		);
	...
	$reconciledStatus = $workflow->getReconciledStatus($object);
	if ($reconciledStatus !== (int) $object->status) {
		$object->status = $reconciledStatus;
	}

	$object->context['trigger_reason'] = 'mes_update';
	$object->context['changed_fields'] = array('mes_required', 'mes_status', 'date_mes', 'date_consuel', 'ref_consuel', 'status');
	$result = !$saveError ? $object->update($user) : -1;
	if ($result > 0) {
		if ($action === 'save') {
			$db->commit();
			if ($uploadResult['result'] > 0 && $oldConsuelFile !== '') {
				$newConsuelPiece = $workflow->getConsuelPiece($object);
				$newConsuelFile = is_object($newConsuelPiece) ? rtrim((string) $newConsuelPiece->filepath, '/').'/'.(string) $newConsuelPiece->filename : '';
				if ($newConsuelFile !== '' && $newConsuelFile !== $oldConsuelFile && is_file($oldConsuelFile)) {
					dol_delete_file($oldConsuelFile);
				}
			}
		}
	...
	if ($action === 'save') {
		$db->rollback();
		if ($uploadResult['result'] > 0 && $uploadResult['filename'] !== '') {
			dol_delete_file(procedurespvGetRaccordementUploadDir($object).'/'.$uploadResult['filename']);
		}
	}
	if (!$saveError) {
...
$mesStatusTypes = array(0 => 'status0', 1 => 'status3', 2 => 'status1', 3 => 'status1', 4 => 'status4', 5 => 'status8', 6 => 'status8');
...
print '</td></tr>';
print '<tr><td class="fieldrequired">'.$langs->trans('ConsuelDate').'</td><td>';
$form->selectDate($object->date_consuel ? (int) $object->date_consuel : -1, 'date_consuel', 1, 1, 1, '', 1, 1);
print '</td></tr>';
print '<tr><td class="fieldrequired">'.$langs->trans('ConsuelReference').'</td><td><input type="text" class="flat minwidth300" name="ref_consuel" required value="'.dol_escape_htmltag((string) $object->ref_consuel).'"></td></tr>';
$consuelPiece = $workflow->getConsuelPiece($object);
$hasConsuelFile = is_object($consuelPiece) && $workflow->pieceFileExists($consuelPiece);
print '<tr><td class="fieldrequired">'.$langs->trans('ConsuelDocument').'</td><td><input type="file" class="flat" name="consuel_file"'.(!$hasConsuelFile ? ' required' : '').'>';
if ($hasConsuelFile) {
	print ' '.procedurespvRenderRaccordementDocumentLink($object, (string) $consuelPiece->filename);
	print ' '.$consuelPiece->getLibStatut(5);
}
print '</td></tr>';
		...
		if (!empty($mesTransitions[$mesAction])) {
			if ($mesAction === 'mark_requested') {
				...
				if (!empty($missingRequirements)) {
					...
					print '<span class="butActionRefused classfortooltip" title="'.dol_escape_htmltag($langs->trans('MESRequestBlocked').' : '.implode(', ', $missingLabels)).'">'.$langs->trans($labelKey).'</span>';
					continue;
				}
			}
